...
metadata: {"version": "2.0", "skill-author": "paleoecology-research", "based-on": "paleoecology-meta-analysis.html"}
---
...
**版本兼容性提示**：SciPy 1.7+ 的 `scipy.stats.bootstrap` 默认使用 BCa 方法。PyGAM 的 `LinearGAM` 支持 GCV 自动平滑参数选择。PyMC 5.x 与 ArviZ 1.x 兼容，贝叶斯 GAM 报告时须显式指定 `ci_prob=0.95`。
...
**模块依赖关系**：`preprocessing` + `continuous_proxy` 被所有场景依赖 → `synthesis` 被场景二、三依赖 → `effect_size` 被场景一、三依赖 → `validation` 被所有场景调用 → `scenarios` 依赖前五个模块。
